...
def carla_video_tracking_dev_canonical_preprocessing(batch):
    ...
    return canonical_variable_image_preprocess(carla_video_tracking_dev_context, batch)
...
def carla_video_tracking_label_preprocessing(x, y):
    ...
    box_array = tf.squeeze(box_labels, axis=0) if box_labels.shape[0] == 1 else box_labels
    box_labels = {"boxes": box_array}
    patch_metadata = {k: (tf.squeeze(v, axis=0) if v.shape[0] == 1 else v) for k, v in patch_metadata.items()}
